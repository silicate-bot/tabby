...
//  2025-06-11: OpenGL: Added support for ImGuiBackendFlags_RendererHasTextures,
//  for dynamic font atlas. Removed ImGui_ImplOpenGL3_CreateFontsTexture() and
...
static GLuint Silicate_BeginPixelUpload() {
  GLuint last_pixel_unpack_buffer = 0;
  ...
  return last_pixel_unpack_buffer;
}

static void Silicate_EndPixelUpload(GLuint last_pixel_unpack_buffer) {
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData *tex) {
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    GLuint last_pixel_unpack_buffer = Silicate_BeginPixelUpload();
    GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex->Width, tex->Height, 0,
                         GL_RGBA, GL_UNSIGNED_BYTE, pixels));
    Silicate_EndPixelUpload(last_pixel_unpack_buffer);
...
#endif
    Silicate_EndPixelUpload(
        last_pixel_unpack_buffer); // Silicate: restore host PBO
    tex->SetStatus(ImTextureStatus_OK);
...
void ImGui_ImplOpenGL3_InvalidateDeviceObjectsForContextLoss() {
  ImGui_ImplOpenGL3_Data *bd = ImGui_ImplOpenGL3_GetBackendData();
  if (bd == nullptr)
    return;

  // Forget shader + buffer handles (no glDelete). CreateDeviceObjects(),
  // invoked from NewFrame() while ShaderHandle==0, regenerates them and
  // recomputes every cached attribute/uniform location.
  bd->VboHandle = 0;
  bd->ElementsHandle = 0;
  bd->ShaderHandle = 0;

  // Forget texture handles (no glDelete) and request re-upload on the new
  // context. The CPU-side pixels stay owned by the atlas, so WantCreate
  // re-uploads cleanly.
  for (ImTextureData *tex : ImGui::GetPlatformIO().Textures) {
    tex->SetTexID(ImTextureID_Invalid);
