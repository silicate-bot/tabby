...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = ImVec2(window->Size.x / 2.0f, 0.0f);
    size.y = label_size.y + style.FramePadding.y * 2.0f; // Default height is based on label height
    // const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    // const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    const ImVec2 frame_size = size;
    ...
    if (render_cursor || render_selection)
    {
        ...
        tabby::DragData& animatedTextPosData = tabby::getStoredAnimValue(label, tabby::DragData{
            .x = draw_pos.x,
            .y = draw_pos.y
        });
        animatedTextPosData.x = tabby::interpolateValue(animatedTextPosData.x, draw_pos.x);
        animatedTextPosData.y = tabby::interpolateValue(animatedTextPosData.y, draw_pos.y);
        ImVec2 animated_draw_pos = ImVec2(animatedTextPosData.x, animatedTextPosData.y);
        animated_draw_pos = ImVec2(animatedTextPosData.x - draw_scroll.x, animatedTextPosData.y - draw_scroll.y);
        if (render_selection)
        {
            ...
            ImVec2 rect_pos = animated_draw_pos + select_start_offset - draw_scroll;
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, animated_draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            ImVec2 cursor_screen_pos = ImTrunc(animated_draw_pos + cursor_offset);
            struct CursorData {
                float x = 0.0f;
                float y = 0.0f;
            };
            CursorData& animatedCursorData = tabby::getStoredAnimValue("INTERNAL_inputtextcursor", CursorData{});
            animatedCursorData.x = tabby::interpolateValue(animatedCursorData.x, cursor_screen_pos.x, 40.0f);
            animatedCursorData.y = tabby::interpolateValue(animatedCursorData.y, cursor_screen_pos.y, 40.0f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    SelectableData& animated = tabby::getStoredAnimValue(label, SelectableData{
        .x = pos.x,
        .y = pos.y
    });
    animated.x = tabby::interpolateValue(animated.x, pos.x);
    animated.y = tabby::interpolateValue(animated.y, pos.y);
    pos.x = animated.x;
    pos.y = animated.y;
