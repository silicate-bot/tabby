...
bool ImGui::RadioButton(const char* label, bool active) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddCircle(center + ImVec2(1, 1), radius - pad,
                                    GetColorU32(ImGuiCol_BorderShadow),
                                    num_segment, style.FrameBorderSize);
        window->DrawList->AddCircle(center, radius - pad,
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
    ...
    size.y =
        label_size.y +
        style.FramePadding.y * 2.0f;  // Default height is based on label height
    ...
    // * 2.0f); // Arbitrary default of 8 lines high for multi-line const ImVec2
    ...
    if (render_cursor || render_selection) {
        ...
        ImVec2 target_pos = draw_pos - frame_bb.Min;

        tabby::DragData& animated = tabby::getStoredAnimValue(
            label, tabby::DragData{.x = target_pos.x, .y = target_pos.y});
        animated.x = tabby::interpolateValue(animated.x, target_pos.x);
        animated.y = tabby::interpolateValue(animated.y, target_pos.y);
        ImVec2 animated_draw_pos =
            ImVec2(animated.x, animated.y) - draw_scroll + frame_bb.Min;
        if (render_selection) {
            ...
            ImVec2 rect_pos =
                animated_draw_pos + select_start_offset - draw_scroll;
                ...
                if (rect_pos.y < clip_rect.y) {
                    ...
                    if (rect_size.x <= 0.0f)
                        rect_size.x = IM_TRUNC(
                            g.FontBaked->GetCharAdvance((ImWchar)' ') *
        ...
        if (render_cursor) {
            ...
            ImVec2 cursor_screen_pos =
                ImTrunc(animated_draw_pos + cursor_offset);
            struct CursorData {
                float x = 0.0f;
                float y = 0.0f;
            };
            CursorData& animatedCursorData = tabby::getStoredAnimValue(
                "INTERNAL_inputtextcursor", CursorData{});
            animatedCursorData.x = tabby::interpolateValue(
                animatedCursorData.x, cursor_screen_pos.x, 40.0f);
            animatedCursorData.y = tabby::interpolateValue(
                animatedCursorData.y, cursor_screen_pos.y, 40.0f);
            cursor_screen_pos.x = animatedCursorData.x;
            cursor_screen_pos.y = animatedCursorData.y;
            ...
            // This is required for some backends (SDL3) to start emitting
            // character/text inputs. As per #6341, make sure we don't set that
            // on the deactivating frame.
...
bool ImGui::ColorEdit4(const char* label, float col[4],
    ...
    const ImGuiStyle& style = g.Style;
    const float square_sz = GetFrameHeight();
    const auto total_width = tabby::TabbyGlobalCfg::get().widgetWidth;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
        ...
        window->DC.CursorPos = ImVec2(pos.x + total_width - square_sz, pos.y);
...
bool ImGui::Selectable(const char* label, bool selected,
    ...
    // selectables are mostly used in lists so yea
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset;
    ...
    SelectableData& animated = tabby::getStoredAnimValue(
        label, SelectableData{.x = pos.x, .y = pos.y});
    animated.x = tabby::interpolateValue(animated.x, pos.x, 50.0f);
    animated.y = tabby::interpolateValue(animated.y, pos.y, 50.0f);
    pos.x = animated.x;
    pos.y = animated.y;
...
void ImGui::EndMainMenuBar() {
    ...
    if (!g.CurrentWindow->DC.MenuBarAppending) {
        IM_ASSERT_USER_ERROR(0,
                             "Calling EndMainMenuBar() not from a "
                             "menu-bar!");  // Not technically testing that
